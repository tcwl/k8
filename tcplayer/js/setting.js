    ...
    if( danmuon > 0){
	var dp = new tcplayer({autoplay: autoplay,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},danmaku: {id: dmid,api: dmapi,user: user,}});
	}else{$('body').addClass("danmu-off");
	var dp = new tcplayer({autoplay: autoplay,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},});}
    // 弹出框
    //function alert_back(text) {$(".alert_back").html(text).show();setTimeout(function () { $(".alert_back").fadeOut('slow'); },1200)}
    // 通用点击
	add('.tcplayer-list-icon',".tcplayer-danmu",'show');
    ...
    $(".tcplayer-showing").on("click", function () {dp.play();$(".vod-pic").remove();});
    //个性化弹幕框		
	$(".tcplayer-comment-setting-color input").on("click", function () {
    var textcolor = $(this).attr("value"); 
    setTimeout(function (){$('.tcwl-tcplayer-comment-input').css({"color":textcolor});}, 100);
    });
    $(".tcplayer-comment-setting-type input").on("click", function () {
    var texttype = $(this).attr("value"); 
    setTimeout(function (){$('.tcwl-tcplayer-comment-input').attr("dmtype",texttype);}, 100);
    });
    $("#dmset").on("click", function () {
    $(".tcplayer-comment-icon").trigger("click");
    $(".tcplayer-comment-setting-box").toggleClass("tcplayer-comment-setting-open") 
    $("#tcplayer").toggleClass("tcplayer-hide-controller") 
    });

    $(".tcwl-tcplayer-send-icon").on("click", function () {
    var inputtext = document.getElementById("dmtext");
    var sendtexts = inputtext.value;
    var sendtype =$('.tcwl-tcplayer-comment-input').attr("dmtype");
    var sendcolor = $('.tcplayer-comment-input').css("color"); 
    ...
    $(".tcwl-tcplayer-comment-input").val("");
    })

$('#dmtext').on('keyup', function (e) {
      if (e.keyCode == 13) {
        console.log('回车')
        $(".tcwl-tcplayer-send-icon").trigger("click")
      }
...
$(".tcplayer-list-icon,.tcwl-tcplayer-send-icon").on("click", function () {		   
	$(".list-show").empty();
	$.ajax({
    url:dmapi+"?id="+dmid,
    success:function (data) {
         ...
         $(".tcplayer-fulloff-icon").on("click", function () {dp.fullScreen.cancel();})
    //播放loading元素		
function video_play() {
...
function video_con_play() {
		...
		timer = setInterval(function(){
         ...
         var cplayer = `<div class="memory-play-wrap"><div class="memory-play"><span class="close">×</span><span>上次看到 </span><span>${ctime}</span><span class="play-jump">跳转播放</span></div></div>`
             $(".tcplayer-cplayer").append(cplayer);
