    ...
    setTimeout(function (){$('.tc_wl-tcplayer-comment-input').css({"color":textcolor});}, 100);
    });
    $(".tcplayer-comment-setting-type input").on("click", function () {
    var texttype = $(this).attr("value"); 
    setTimeout(function (){$('.tc_wl-tcplayer-comment-input').attr("dmtype",texttype);}, 100);
    });
    ...
    $(".tc_wl-tcplayer-send-icon").on("click", function () {
    var inputtext = document.getElementById("dmtext");
    var sendtexts = inputtext.value;
    var sendtype =$('.tc_wl-tcplayer-comment-input').attr("dmtype");
...
$(".tcplayer-list-icon,.tc_wl-tcplayer-send-icon").on("click", function () {		   
	$(".list-show").empty();
	$.ajax({
    url:dmapi+"?id="+dmid,
